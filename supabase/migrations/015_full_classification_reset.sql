...
-- A deliberate full reclassification is different from the normal incremental
-- pipeline. It removes every previous Jev run/result for one mailbox and clears
-- decisions that would otherwise override the fresh model output. The private
-- labeling JSON, reply drafts, application stars, and email source data are not
-- part of this reset.
...
begin
  ...
  -- email_classifications are deleted by the classification_runs FK cascade.
...
comment on function public.reset_account_classification_state(uuid) is
  'Destructive account-scoped reset used only by an explicit full classification run; preserves source email, private label files, drafts, stars, and application rows.';
